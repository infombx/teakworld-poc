...
import { AboutBlock as AboutBlockType, getImageUrl } from '@/lib/strapi';
...
export default function AboutBlock({ block }: AboutBlockProps) {
    const imageUrl = block.image ? getImageUrl(block.image) : 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800&q=80';
            ...
            <div className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex flex-col lg:flex-row items-center gap-12 lg:gap-20">
                    ...
                    <div className="w-full lg:w-1/2 relative">
                        ...
                        {block.stats && block.stats.length > 0 && (
                            <div className="absolute -bottom-6 -right-6 w-48 h-48 bg-white p-4 rounded-xl shadow-xl hidden md:flex flex-col items-center justify-center text-center">
                                <span className="text-4xl font-black text-accent font-display">{block.stats[0].value}</span>
                    ...
                    <div className="w-full lg:w-1/2 flex flex-col gap-6">
                        {block.subtitle && (
                            <span className="text-accent font-bold tracking-widest text-sm uppercase">{block.subtitle}</span>
                        )}
